...
class SortingTests(unittest.TestCase):
    def test_selection_sort(self):
        f = FlowGen("test_files/sorts.py", "selection_sort", [64, 25, 12])
        timeline = f.generate_flowchart(visual=False)
        expected_blocks = [2, 6, 10, 11, 12, 13, 11, 12, 13, 11, 18, 6, 10, 11, 12, 11, 18, 6, 10, 11, 18, 6]
        ans = [block.at() for block in timeline]
        print(ans,"####")
        for actual, expected in zip(timeline, expected_blocks):
            self.assertEqual(actual.at(), expected)
    def test_insertion_sort(self):
        ...
        timeline = f.generate_flowchart(visual=False)
        expected_blocks = [2, 23, 25, 31, 32, 31, 34, 23, 25, 31, 32, 31, 32, 31, 34, 23]
        for actual, expected in zip(timeline, expected_blocks):
            self.assertEqual(actual.at(), expected)
